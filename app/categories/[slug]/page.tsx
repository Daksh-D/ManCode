// app/categories/[slug]/page.tsx (CORRECTED)
...
async function fetchProductsByCategory(category: string): Promise<Product[]> {
    ...
    const allProducts: {products: Product[]} = await res.json(); //the returned type is not Product[]

    //Now filter products based on this category.
    const products = allProducts.products.filter(prod=> prod.category.toLowerCase() === category );
    return products;
}
